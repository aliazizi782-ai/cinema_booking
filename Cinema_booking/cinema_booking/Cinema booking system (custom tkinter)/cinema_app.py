...
def change_window(hide_window: ctk.CTkToplevel, show_window: ctk.CTkToplevel):
    hide_window.withdraw()

    if hide_window == movie_window:
        temp = [combo_movie_id_movie_window, entry_title_movie_window, entry_director_movie_window,
                entry_genre_movie_window, entry_duration_movie_window, entry_release_date_movie_window,
                entry_rating_movie_window]
        clear_entries(temp)
    ...
    if hide_window == screen_window:
        temp = [combo_screen_id_screen_window, combo_movie_screen_window, combo_hall_screen_window,
                entry_screening_datetime_screen_window, entry_ticket_price_screen_window]
        clear_entries(temp)

    if hide_window == sign_up_window:
        temp = [entry_name_sign_up_window, entry_surname_sign_up_window, entry_username_sign_up_window,
                entry_password_sign_up_window, entry_password2_sign_up_window, entry_access_level_sign_up_window]
        clear_entries(temp)

    if show_window == sign_up_window:
        setup_sign_up_window(hide_window)
    if show_window == main_menu:
        setup_main_menu()
    if show_window == view_reservation_window:
        setup_view_reservation_window()
    if show_window == movie_window:
        setup_movie_window()
    if show_window == screen_window:
        setup_screen_window()
    if show_window == hall_window:
        setup_hall_window()
    if show_window == reserve_movie_window:
        setup_reserve_movie_window()
    if show_window == users_window:
        setup_users_window()

    show_window.deiconify()

def setup_sign_up_window(hide_window: ctk.CTkToplevel):
    ...
    if hide_window == root:
        ...
        btn_ok_sign_up_window.configure(command=insert_user)
        btn_back_sign_up_window.configure(command=lambda: change_window(sign_up_window, root))
        ...
        btn_ok_sign_up_window.configure(command=insert_user)
        btn_back_sign_up_window.configure(command=lambda: change_window(sign_up_window, users_window))

    elif hide_window == main_menu:
        sign_up_window.title("Profile window")
        btn_back_sign_up_window.configure(command=lambda: change_window(sign_up_window, main_menu))
        btn_ok_sign_up_window.configure(command=edit_profile)
        if access_level == 1:
            lbl_access_level_sign_up_window.grid(**CNF_GRID_GENERAL, row=6, column=1)
            entry_access_level_sign_up_window.grid(**CNF_GRID_GENERAL, row=6, column=2)
        else:
...
def setup_main_menu():
    ...
    if access_level == 2:
        ...
        btn_view_reservation_main_menu.configure(text="View my reservations")
    else:
        ...
        btn_view_reservation_main_menu.configure(text="View reservations")
...
def setup_view_reservation_window():
    ...
    result = sorted([str(item[0]) for item in result])
    combo_reservation_id_view_reservation_window.configure(values=[""] + result)
    ...
    if access_level == 1:
        ...
        entry_username_view_reservation_window.configure(state="disabled")

def setup_movie_window():
    search_movie()
    result = connection.set_combo_movie_id_movie_window_values()
    result = [str(item[0]) for item in result]
    combo_movie_id_movie_window.configure(values=[""] + result)
...
def setup_screen_window():
    ...
    result1 = [str(item[0]) for item in result1]
    result2 = [str(item[0]) for item in result2]
    result3 = [str(item[0]) for item in result3]
    combo_hall_screen_window.configure(values=[""] + result1)
    combo_movie_screen_window.configure(values=[""] + result2)
    combo_screen_id_screen_window.configure(values=[""] + result3)
...
def setup_hall_window():
    ...
    combo_list_hall_window.configure(values=result5)
...
def setup_users_window():
    ...
    result2 = sorted([str(item[0]) for item in list(result1)])
    combo_user_id_users_window.configure(values=[""] + result2)
    result3 = [str(item[1]) for item in result1]
    combo_username_users_window.configure(values=[""] + result3)
...
def insert_user():
    ...
    temp = [entry_name_sign_up_window, entry_surname_sign_up_window, entry_username_sign_up_window,
            entry_password_sign_up_window, entry_password2_sign_up_window]
    clear_entries(temp)
...
def search_reservation():
    ...
    result = connection.search_reservation(user_id, access_level, reservation_id, username, screen_id,
                                           seat_count, reserved_at, ticket_price, screening_datetime,
                                           hall, movie, director, duration, rating)
...
def add_reservation():
    ...
    if username == "" or screen_id == "" or seat_count == "":
        msb.showwarning("FormError", "You should enter username, ID screen and seat counts correctly.")
        return
    if temp[0] != 1:
    ...
    if not seat_count.isdigit():
        msb.showerror("Error", "Seat count must be a number.")
        return
...
def get_reservation_data(event):
    reservation_id = combo_reservation_id_view_reservation_window.get().strip()
    result = connection.get_reservation_data(reservation_id)

    temp = [entry_username_view_reservation_window, entry_screen_id_view_reservation_window,
            entry_seat_count_view_reservation_window]
    clear_entries(temp)
    if result[0] == 1:
        result = result[1]
...
def update_reservation():
    ...
    if reservation_id == "":
        ...
        return
    if username == "" or screen_id == "" or seat_count == "":
        msb.showwarning("FormError", "You should fill all fields.")
        return
...
def delete_reservation():
    ...
    if reservation_id == "" or screen_id == "":
        msb.showerror("RemovingError", "Enter reservation ID and screen ID correctly.")
        return
...
def insert_movie():
    ...
    if title == "" or director == "" or genre == "" or duration == "" or release_date == "" or rating == "":
        msb.showwarning("InsertError", "Fill all fields.")
        return
    ...
    try:
        ...
        msb.showwarning("Invalid datetime", "Format must be YYYY-MM-DD HH:MM:SS")
        return
...
def get_movie_data(event):
    movie_id = combo_movie_id_movie_window.get().strip()
    result = connection.get_movie_data(movie_id)

    temp = [entry_title_movie_window, entry_director_movie_window, entry_genre_movie_window,
            entry_duration_movie_window, entry_release_date_movie_window, entry_rating_movie_window]
    clear_entries(temp)
    if result[0] == 1:
        result = result[1]
...
def update_movie():
    ...
    if movie_id == "" or title == "" or director == "" or genre == "" or duration == "" or release_date == "" or rating == "":
        msb.showwarning("FormError", "Fill all fields.")
        return
...
def delete_movie():
    ...
    if movie_id == "" or title == "":
        msb.showerror("RemovingError", "Enter movie ID and title correctly.")
        return
...
def add_hall():
    ...
    try:
        ...
        msb.showerror("CapacityError", "Capacity must be integer.")
        return
...
def reserve_ticket():
    ...
    if result[0] == 0:
        ...
        lst_entry_screen_info_reserve_movie_window = [
            entry_screen_id_reserve_movie_window,
...
def add_screen():
    ...
    if movie == "" or hall == "" or screening_datetime == "" or ticket_price == "":
        msb.showerror("ScreenError", "All fields are required.")
        return
    try:
    ...
    if not hall_free:
        msb.showwarning("Hall free checking went wrong!", "Hall already reserved or invalid data.")
        return
...
def update_screen():
    ...
    if screen_id == "" or movie == "" or hall == "" or screening_datetime == "" or ticket_price == "":
        msb.showwarning("UpdatingError", "Fill all fields.")
        return
    ...
    if not hall_free:
        msb.showwarning("Wrong hall selected", "Hall already reserved.")
        return
...
def delete_screen():
    ...
    if screen_id == "" or movie == "":
        msb.showerror("RemovingError", "Enter screen ID and movie.")
        return
...
def get_user_data(event):
    ...
    if result[1] is not None:
        ...
        entry_name_users_window.insert(0, "")
    if result[2] is not None:
        entry_surname_users_window.insert(0, result[2])
    else:
        entry_surname_users_window.insert(0, "")

    combo_username_users_window.set(str(result[3]))
    combo_access_level_users_window.set(str(result[4]))
...
def delete_user():
    ...
    if user_id == "" or username == "":
        msb.showerror("RemovingError", "Enter user ID and username.")
        return
...
root.configure(**CNF_WINDOW)

lbl_welcome_root = ctk.CTkLabel(root, **CNF_LBL_ROOT, text="Welcome to Cinema Booking Service!")
btn_sign_up_root = ctk.CTkButton(root, **CNF_BTN_ROOT, text="Sign up", command=lambda: change_window(root, sign_up_window))
btn_login_root = ctk.CTkButton(root, **CNF_BTN_ROOT, text="Login", command=lambda: change_window(root, login_window))
...
# ===== Sign up window =====
sign_up_window = ctk.CTkToplevel(root)
...
sign_up_window.configure(**CNF_WINDOW)

lbl_name_sign_up_window = ctk.CTkLabel(sign_up_window, **CNF_LBL_SIGN_UP_WINDOW, text="Name: ")
lbl_surname_sign_up_window = ctk.CTkLabel(sign_up_window, **CNF_LBL_SIGN_UP_WINDOW, text="Surname: ")
lbl_username_sign_up_window = ctk.CTkLabel(sign_up_window, **CNF_LBL_SIGN_UP_WINDOW, text="Username: ")
lbl_password_sign_up_window = ctk.CTkLabel(sign_up_window, **CNF_LBL_SIGN_UP_WINDOW, text="Password: ")
lbl_password2_sign_up_window = ctk.CTkLabel(sign_up_window, **CNF_LBL_SIGN_UP_WINDOW, text="Repeat Password: ")
lbl_access_level_sign_up_window = ctk.CTkLabel(sign_up_window, **CNF_LBL_SIGN_UP_WINDOW, text="Access level: ")
entry_name_sign_up_window = ctk.CTkEntry(sign_up_window, **CNF_ENTRY_GENERAL)
entry_surname_sign_up_window = ctk.CTkEntry(sign_up_window, **CNF_ENTRY_GENERAL)
entry_username_sign_up_window = ctk.CTkEntry(sign_up_window, **CNF_ENTRY_GENERAL)
entry_password_sign_up_window = ctk.CTkEntry(sign_up_window, **CNF_ENTRY_GENERAL)
entry_password2_sign_up_window = ctk.CTkEntry(sign_up_window, **CNF_ENTRY_GENERAL)
entry_access_level_sign_up_window = ctk.CTkEntry(sign_up_window, **CNF_ENTRY_GENERAL)
btn_ok_sign_up_window = ctk.CTkButton(sign_up_window, **CNF_BTN_SIGN_UP_WINDOW, text="OK", command=insert_user)
btn_back_sign_up_window = ctk.CTkButton(sign_up_window, **CNF_BTN_SIGN_UP_WINDOW, text="Back")
...
login_window.configure(**CNF_WINDOW)

lbl_username_login_window = ctk.CTkLabel(login_window, **CNF_LBL_LOGIN_WINDOW, text="Username: ")
lbl_password_login_window = ctk.CTkLabel(login_window, **CNF_LBL_LOGIN_WINDOW, text="Password: ")
entry_username_login_window = ctk.CTkEntry(login_window, **CNF_ENTRY_GENERAL)
entry_password_login_window = ctk.CTkEntry(login_window, **CNF_ENTRY_GENERAL, show="*")
btn_login_window = ctk.CTkButton(login_window, **CNF_BTN_LOGIN_WINDOW, text="Login", command=login)
btn_back_login_window = ctk.CTkButton(login_window, **CNF_BTN_LOGIN_WINDOW, text="Back", command=lambda: change_window(login_window, root))
...
btn_back_login_window.grid(**CNF_GRID_GENERAL, row=3, column=2, sticky="e")

# ===== Main menu =====
main_menu = ctk.CTkToplevel(root)
main_menu.geometry("+50+50")
main_menu.resizable(False, False)
main_menu.withdraw()
main_menu.protocol("WM_DELETE_WINDOW", root.destroy)
main_menu.title("Main menu")
main_menu.configure(**CNF_WINDOW)

btn_view_reservation_main_menu = ctk.CTkButton(main_menu, **CNF_BTN_MAIN_MENU, command=lambda: change_window(main_menu, view_reservation_window))
btn_manage_movies_main_menu = ctk.CTkButton(main_menu, **CNF_BTN_MAIN_MENU, text="Movies", command=lambda: change_window(main_menu, movie_window))
btn_manage_halls_main_menu = ctk.CTkButton(main_menu, **CNF_BTN_MAIN_MENU, text=" Halls", command=lambda: change_window(main_menu, hall_window))
btn_manage_screens_main_menu = ctk.CTkButton(main_menu, **CNF_BTN_MAIN_MENU, text=" Screens", command=lambda: change_window(main_menu, screen_window))
btn_manage_users_main_menu = ctk.CTkButton(main_menu, **CNF_BTN_MAIN_MENU, text=" Users", command=lambda: change_window(main_menu, users_window))
btn_profile_main_menu = ctk.CTkButton(main_menu, **CNF_BTN_MAIN_MENU, text=" My Profile", command=lambda: change_window(main_menu, sign_up_window))
btn_log_out_main_menu = ctk.CTkButton(main_menu, **CNF_BTN_MAIN_MENU, text="Log Out", command=lambda: change_window(main_menu, login_window))
...
view_reservation_window = ctk.CTkToplevel(root)
...
view_reservation_window.configure(**CNF_WINDOW)
...
scr_view_reservation_window = ttk.Scrollbar(view_reservation_window, orient="vertical", command=treev_view_reservation_window.yview)
treev_view_reservation_window.configure(yscrollcommand=scr_view_reservation_window.set)
...
frame_options_reservation_window = ctk.CTkFrame(view_reservation_window, **CNF_LABLE_FRAME)
frame_options_reservation_window.configure(fg_color=BG,  border_width=2, corner_radius=8)
frame_btn_reservation_window = ctk.CTkFrame(view_reservation_window, **CNF_FRAME)


for e in range(4):
    for w in range(3):
        label = ctk.CTkLabel(frame_options_reservation_window, **CNF_LBL_VIEW_RESERVATION_WINDOW, text=columns_treev_view_reservation_window[3*e+w]+": ")
        label.grid(**CNF_GRID_GENERAL, row=e+1, column=2*w+1, sticky="w")

combo_reservation_id_view_reservation_window = ctk.CTkComboBox(frame_options_reservation_window, width=200, state="readonly")
combo_reservation_id_view_reservation_window.bind("<<ComboboxSelected>>", get_reservation_data)
entry_username_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)
entry_screen_id_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)
entry_seat_count_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)
entry_reserved_at_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)
entry_ticket_price_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)
entry_screening_datetime_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)
entry_hall_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)
entry_movie_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)
entry_director_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)
entry_duration_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)
entry_rating_view_reservation_window = ctk.CTkEntry(frame_options_reservation_window, **CNF_ENTRY_GENERAL)

btn_search_refresh_view_reservation_window = ctk.CTkButton(frame_btn_reservation_window, **CNF_BTN_VIEW_RESERVATION_WINDOW, text="Search & refresh", command=search_reservation)
btn_add_view_reservation_window = ctk.CTkButton(frame_btn_reservation_window, **CNF_BTN_VIEW_RESERVATION_WINDOW, text="Add reservation", command=add_reservation)
btn_update_view_reservation_window = ctk.CTkButton(frame_btn_reservation_window, **CNF_BTN_VIEW_RESERVATION_WINDOW, text="Update reservation", command=update_reservation)
btn_delete_view_reservation_window = ctk.CTkButton(frame_btn_reservation_window, **CNF_BTN_VIEW_RESERVATION_WINDOW, text="Delete reservation", command=delete_reservation)
btn_back_view_reservation_window = ctk.CTkButton(frame_btn_reservation_window, **CNF_BTN_VIEW_RESERVATION_WINDOW, text="Back", command=lambda: change_window(view_reservation_window, main_menu))
...
# ===== Movie window =====
movie_window = ctk.CTkToplevel(root)
...
movie_window.configure(**CNF_WINDOW)

lbl_movie_id_movie_window = ctk.CTkLabel(movie_window, **CNF_LBL_MOVIE_WINDOW, text="ID movie: ")
lbl_title_movie_window = ctk.CTkLabel(movie_window, **CNF_LBL_MOVIE_WINDOW, text="Title: ")
lbl_director_movie_window = ctk.CTkLabel(movie_window, **CNF_LBL_MOVIE_WINDOW, text="Director: ")
lbl_genre_movie_window = ctk.CTkLabel(movie_window, **CNF_LBL_MOVIE_WINDOW, text="Genre: ")
lbl_duration_movie_window = ctk.CTkLabel(movie_window, **CNF_LBL_MOVIE_WINDOW, text="Duration: ")
lbl_release_date_movie_window = ctk.CTkLabel(movie_window, **CNF_LBL_MOVIE_WINDOW, text="Release date: ")
lbl_rating_movie_window = ctk.CTkLabel(movie_window, **CNF_LBL_MOVIE_WINDOW, text="Rating: ")
combo_movie_id_movie_window = ctk.CTkComboBox(movie_window, width=200, state="readonly")
combo_movie_id_movie_window.bind("<<ComboboxSelected>>", get_movie_data)
entry_title_movie_window = ctk.CTkEntry(movie_window, **CNF_ENTRY_GENERAL)
entry_director_movie_window = ctk.CTkEntry(movie_window, **CNF_ENTRY_GENERAL)
entry_genre_movie_window = ctk.CTkEntry(movie_window, **CNF_ENTRY_GENERAL)
entry_duration_movie_window = ctk.CTkEntry(movie_window, **CNF_ENTRY_GENERAL)
entry_release_date_movie_window = ctk.CTkEntry(movie_window, **CNF_ENTRY_GENERAL)
entry_rating_movie_window = ctk.CTkEntry(movie_window, **CNF_ENTRY_GENERAL)
btn_add_movie_window = ctk.CTkButton(movie_window, **CNF_BTN_MOVIE_WINDOW, text="Add movie", command=insert_movie)
btn_search_movie_window = ctk.CTkButton(movie_window, **CNF_BTN_MOVIE_WINDOW, text="Search & refresh movies", command=search_movie)
btn_update_movie_window = ctk.CTkButton(movie_window, **CNF_BTN_MOVIE_WINDOW, text="Update movie", command=update_movie)
btn_delete_movie_window = ctk.CTkButton(movie_window, **CNF_BTN_MOVIE_WINDOW, text="Delete movie", command=delete_movie)
btn_back_movie_window = ctk.CTkButton(movie_window, **CNF_BTN_MOVIE_WINDOW, text="Back", command=lambda: change_window(movie_window, main_menu))

# Treeview for movies (ttk)
treev_movie_window = ttk.Treeview(movie_window, columns=["1","2","3","4","5","6","7"], show="headings", selectmode="browse")
scr_movie_window = ttk.Scrollbar(movie_window, orient="vertical", command=treev_movie_window.yview)
treev_movie_window.configure(yscrollcommand=scr_movie_window.set)
...
treev_movie_window.grid(**CNF_GRID_GENERAL, row=1, column=4, rowspan=7, sticky="n")
scr_movie_window.grid(**CNF_GRID_GENERAL, row=1, column=5, sticky="ns", rowspan=6)

# ===== Hall window =====
hall_window = ctk.CTkToplevel(root)
...
hall_window.configure(**CNF_WINDOW)

lbl_name_hall_window = ctk.CTkLabel(hall_window, **CNF_LBL_HALL_WINDOW, text="Name: ")
lbl_capacity_hall_window = ctk.CTkLabel(hall_window, **CNF_LBL_HALL_WINDOW, text="Capacity: ")
entry_name_hall_window = ctk.CTkEntry(hall_window, **CNF_ENTRY_GENERAL)
entry_capacity_hall_window = ctk.CTkEntry(hall_window, **CNF_ENTRY_GENERAL)
btn_add_hall_window = ctk.CTkButton(hall_window, **CNF_BTN_HALL_WINDOW, text="Add hall", command=add_hall)
btn_delete_hall_window = ctk.CTkButton(hall_window, **CNF_BTN_HALL_WINDOW, text="Delete hall", command=delete_hall)
btn_back_hall_window = ctk.CTkButton(hall_window, **CNF_BTN_HALL_WINDOW, text="Back", command=lambda: change_window(hall_window, main_menu))
lbl_list_hall_window = ctk.CTkLabel(hall_window, **CNF_LBL_HALL_WINDOW, text="The List of halls: ")
combo_list_hall_window = ctk.CTkComboBox(hall_window, state="readonly", width=200)
...
# ===== Screen window =====
screen_window = ctk.CTkToplevel(root)
...
screen_window.configure(**CNF_WINDOW)

treev_screen_window = ttk.Treeview(screen_window, columns=["1","2","3","4","5","6","7"], show="headings", selectmode="browse")
scr_screen_window = ttk.Scrollbar(screen_window, orient="vertical", command=treev_screen_window.yview)
treev_screen_window.configure(yscrollcommand=scr_screen_window.set)
...
lbl_screen_id_manage_screen_window = ctk.CTkLabel(screen_window, **CNF_LBL_SCREEN_WINDOW, text="ID screen: ")
lbl_movie_manage_screen_window = ctk.CTkLabel(screen_window, **CNF_LBL_SCREEN_WINDOW, text="Movie: ")
lbl_hall_manage_screen_window = ctk.CTkLabel(screen_window, **CNF_LBL_SCREEN_WINDOW, text="Hall: ")
lbl_screening_datetime_manage_screen_window = ctk.CTkLabel(screen_window, **CNF_LBL_SCREEN_WINDOW, text="Screening datetime: ")
lbl_ticket_price_manage_screen_window = ctk.CTkLabel(screen_window, **CNF_LBL_SCREEN_WINDOW, text="Ticket price: ")
combo_screen_id_screen_window = ctk.CTkComboBox(screen_window, width=200, state="readonly")
combo_screen_id_screen_window.bind("<<ComboboxSelected>>", get_screen_data)
combo_movie_screen_window = ctk.CTkComboBox(screen_window, width=200, state="readonly")
combo_hall_screen_window = ctk.CTkComboBox(screen_window, width=200, state="readonly")
entry_screening_datetime_screen_window = ctk.CTkEntry(screen_window, **CNF_ENTRY_GENERAL)
entry_ticket_price_screen_window = ctk.CTkEntry(screen_window, **CNF_ENTRY_GENERAL)
btn_reserve_ticket_screen_window = ctk.CTkButton(screen_window, **CNF_BTN_SCREEN_WINDOW, text="Reserve Ticket", command=reserve_ticket)
btn_add_screen_screen_window = ctk.CTkButton(screen_window, **CNF_BTN_SCREEN_WINDOW, text="Add Screen", command=add_screen)
btn_update_screen_screen_window = ctk.CTkButton(screen_window, **CNF_BTN_SCREEN_WINDOW, text="Update Screen", command=update_screen)
btn_delete_screen_screen_window = ctk.CTkButton(screen_window, **CNF_BTN_SCREEN_WINDOW, text="Delete Screen", command=delete_screen)
btn_search_refresh_screens_screen_window = ctk.CTkButton(screen_window, **CNF_BTN_SCREEN_WINDOW, text="Search & Refresh Screen", command=search_screen, font=("tahoma", 13, "bold"))
btn_back_screen_window = ctk.CTkButton(screen_window, **CNF_BTN_SCREEN_WINDOW, text="Back", command=lambda: change_window(screen_window, main_menu))
...
reserve_movie_window = ctk.CTkToplevel(root)
...
reserve_movie_window.configure(**CNF_WINDOW)

lbl_screen_info_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Screen Information: ")
lbl_screen_id_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="ID screen: ")
lbl_screening_datetime_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Screening datetime: ")
lbl_hall_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Hall: ")
lbl_ticket_price_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Ticket price: ")
entry_screen_id_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
entry_screening_datetime_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
entry_hall_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
entry_ticket_price_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="_____________________________________________").grid(**CNF_GRID_LBL_RESERVE_MOVIE_WINDOW, row=6, column=1)

lbl_movie_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Movie: ")
lbl_director_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Director: ")
lbl_duration_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Duration: ")
lbl_genre_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Genre")
lbl_release_date_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Release date: ")
lbl_rating_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Rating: ")
entry_movie_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
entry_director_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
entry_duration_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
entry_genre_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
entry_release_date_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
entry_rating_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="_____________________________________________").grid(**CNF_GRID_LBL_RESERVE_MOVIE_WINDOW, row=13, column=1)

lbl_seat_count_reserve_movie_window = ctk.CTkLabel(reserve_movie_window, **CNF_LBL_RESERVE_MOVIE_WINDOW, text="Enter seat count: ", font=("tahoma", 16, "bold"))
entry_seat_count_reserve_movie_window = ctk.CTkEntry(reserve_movie_window, **CNF_ENTRY_GENERAL)
btn_ok_reserve_movie_window = ctk.CTkButton(reserve_movie_window, **CNF_BTN_RESERVE_MOVIE_WINDOW, text="Ok", command=check_and_reserve)
btn_back_reserve_movie_window = ctk.CTkButton(reserve_movie_window, **CNF_BTN_RESERVE_MOVIE_WINDOW, text="Back", command=lambda: change_window(reserve_movie_window, screen_window))

lbl_screen_info_reserve_movie_window.grid(**CNF_GRID_LBL_RESERVE_MOVIE_WINDOW, row=1, column=1)
...
# ===== Users window =====
users_window = ctk.CTkToplevel(root)
...
users_window.configure(**CNF_WINDOW)

treev_users_window = ttk.Treeview(users_window, columns=["1","2","3","4","5"], show="headings", selectmode="browse")
scr_users_window = ttk.Scrollbar(users_window, orient="vertical", command=treev_users_window.yview)
treev_users_window.configure(yscrollcommand=scr_users_window.set)
...
frame_options_users_window = ctk.CTkFrame(users_window, **CNF_LABLE_FRAME)
frame_options_users_window.configure(fg_color=BG, border_width=2, corner_radius=8)
frame_btn_users_window = ctk.CTkFrame(users_window, **CNF_FRAME)

lbl_user_id_users_window = ctk.CTkLabel(frame_options_users_window, **CNF_LBL_USERS_WINDOW, text="ID user")
lbl_name_users_window = ctk.CTkLabel(frame_options_users_window, **CNF_LBL_USERS_WINDOW, text="Name")
lbl_surname_users_window = ctk.CTkLabel(frame_options_users_window, **CNF_LBL_USERS_WINDOW, text="Surname")
lbl_username_users_window = ctk.CTkLabel(frame_options_users_window, **CNF_LBL_USERS_WINDOW, text="Username")
lbl_access_level_users_window = ctk.CTkLabel(frame_options_users_window, **CNF_LBL_USERS_WINDOW, text="Access level")
combo_user_id_users_window = ctk.CTkComboBox(frame_options_users_window, width=200, state="readonly")
combo_user_id_users_window.bind("<<ComboboxSelected>>", get_user_data)
entry_name_users_window = ctk.CTkEntry(frame_options_users_window, **CNF_ENTRY_GENERAL)
entry_surname_users_window = ctk.CTkEntry(frame_options_users_window, **CNF_ENTRY_GENERAL)
combo_username_users_window = ctk.CTkComboBox(frame_options_users_window, width=200, state="readonly")
combo_access_level_users_window = ctk.CTkComboBox(frame_options_users_window, width=200, values=["", "1", "2"], state="readonly")

btn_search_refresh_users_window = ctk.CTkButton(frame_btn_users_window, **CNF_BTN_USERS_WINDOW, text="Search & refresh", command=search_user)
btn_add_users_window = ctk.CTkButton(frame_btn_users_window, **CNF_BTN_USERS_WINDOW, text="Add user", command=lambda: change_window(users_window, sign_up_window))
btn_update_users_window = ctk.CTkButton(frame_btn_users_window, **CNF_BTN_USERS_WINDOW, text="Update user", command=update_user)
btn_delete_users_window = ctk.CTkButton(frame_btn_users_window, **CNF_BTN_USERS_WINDOW, text="Delete user", command=delete_user)
btn_back_users_window = ctk.CTkButton(frame_btn_users_window, **CNF_BTN_USERS_WINDOW, text="Back", command=lambda: change_window(users_window, main_menu))
